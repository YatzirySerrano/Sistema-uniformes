...
- **CATÁLOGOS COMPARTIDOS (Etapa 1 / Bloque C).** `tipos_activo`, `categorias_activo` y `tallas` son catálogos **reutilizables a nivel plataforma**: NO tienen `empresa_id`. Se **habilitan por empresa** con pivotes N:M (`tipo_activo_empresa`, `categoria_activo_empresa`, `talla_empresa`). Catálogo compartido ≠ inventario compartido: el activo pertenece a una empresa y el stock se llavea por `empresa + almacén + activo + talla`; que "M" o "Prenda" sean la misma fila para SIESA e INMAG no mezcla sus activos ni sus saldos. Modelos: `empresas()` (BelongsToMany), `scopeParaEmpresa($q, $empresaId)`, `habilitado/aPara(int)`.
- **Tipo de activo** (`tipos_activo`): `TipoActivoController`, pantalla `Activos/Catalogos.vue`, permiso `tipos-activo.administrar`. `activo` = estado **global** (retira de nuevas selecciones en todas las empresas); el pivote define para quién se ofrece. En el formulario de Activo: **combobox con búsqueda** (`GET tipos-activo/buscar?empresa_id=&q=` — filtra por pivote; sin `empresa_id` → todas las autorizadas; `empresa_id` inválido/sin acceso → `[]`) con **"+ Crear nuevo tipo"** en el desplegable → diálogo → `POST tipos-activo/rapido` (JSON, habilita SÓLO la empresa del formulario). Alta desde la pantalla de catálogos: `POST tipos-activo` con `empresa_ids[]`. Habilitación por empresa: `PUT tipos-activo/{tipo}/empresas` (`empresa_ids[]`). **No existe "Uniforme / Prenda"** (migración `..._000021` lo absorbe en "Prenda").
- **Categoría de activo** (`categorias_activo`): igual patrón (`CategoriaActivoController`, `categorias-activo.administrar`, `PUT categorias-activo/{categoria}/empresas`). `tipo_activo_id` nullable (relación **opcional**, al tipo compartido). `GET categorias-activo/buscar?empresa_id=&q=&tipo_activo_id=` prioriza y acota al tipo + las categorías sin tipo. `activos.categoria_id` es la fuente de verdad; `activos.categoria` (texto) es espejo temporal sincronizado por `ActivoController`.
- **Tipo y categoría son OPCIONALES e independientes**: un activo se guarda con ambos, sólo uno, o ninguno. Coherencia (backend `GuardarActivoRequest::withValidator` + UI): si el activo lleva tipo y categoría, y la categoría está ligada a un tipo concreto, deben coincidir. El activo de la empresa A sólo puede usar tipo/categoría/variante **habilitados para A** (reglas `Rule::exists('tipo_activo_empresa'|'categoria_activo_empresa'|'talla_empresa', ...)->where('empresa_id', ...)`).
- **Unicidad normalizada** de nombres de catálogo: `tipos_activo.nombre_normalizado` / `categorias_activo.nombre_normalizado` / `tallas.valor_normalizado` con índice único **a nivel plataforma** (ya no por empresa). "Prenda" / " prenda " / "PRENDA" son el mismo registro; distintas empresas ya **no** pueden repetir nombre (comparten el catálogo). Sincronizado en `saving` por `App\Models\Concerns\NombreNormalizado` (columna configurable) + `App\Soporte\NormalizadorNombre::catalogo()`.
- **Desactivar globalmente** un tipo/categoría/variante (diálogo de confirmación en `Activos/Catalogos.vue` / `Activos/Tallas.vue`) lo retira de nuevas selecciones en **todas** las empresas; **deshabilitar por empresa** (checkbox "Disponible en «{empresa}»", toggle `POST .../empresa`, o diálogo "N empresas" con la lista completa) sólo para esa empresa. Todo mensaje **nombra la empresa afectada**. Nunca borra ni pone la FK a null: los activos existentes conservan su tipo/categoría/variante y `edit` los muestra aunque estén inactivos/deshabilitados (`seleccion` + `tallasAsignadas` en el payload; las variantes asignadas ya deshabilitadas se ven marcadas para poder quitarlas).
- **Variante elegible de un activo en una empresa** = asociada al activo (`activo_talla`) **y** habilitada para esa empresa (`talla_empresa`) **y** `activa`. Fuente única: `Activo::tallasHabilitadas(int $empresaId)`. `GET activos/buscar` devuelve `tallas` (sólo elegibles) + `usa_variantes` (tiene alguna asociada). Un activo con variantes pero ninguna elegible para la empresa se rechaza en "Registrar entrada" (backend + UI). **Ajuste / mínimos** NO exigen "habilitada" (sólo asociada al activo o nula): hay que poder corregir existencias históricas.
- **Activo** (`activos`, antes `prendas`; pivote `activo_talla`, FK `activo_id` en inventario y detalles): `codigo` `ACT-0001`; `tipo_activo_id` → `tipos_activo`; `categoria_id` → `categorias_activo`; `tipo_control` (`cantidad` | `serializado`). El activo **sí** conserva `empresa_id`. El flujo de **unidades serializadas** (serie/IMEI, entidad `UnidadActivo`) sigue **pendiente** (Etapa 2).
...
- **Bloque B — HECHO**: tipo/categoría opcionales e independientes; buscadores `tipos-activo/buscar` y `categorias-activo/buscar` (dependencia `empresa_id`); alta inline dentro del combobox (fin de los botones "Otro/Otra"); unicidad normalizada (`nombre_normalizado`); diálogo de confirmación al activar/desactivar; filtros Tipo/Categoría searchable en el listado de Activos.
- **Bloque C · Etapa 1 (Catálogos compartidos) — HECHO**: tipos/categorías/variantes sin `empresa_id`, habilitados por pivote N:M; unicidad normalizada de plataforma; alta inline habilita la empresa actual; pantalla `Activos/Catalogos.vue` con habilitación por empresa; `Activos/Tallas.vue` con catálogo compartido + toggle por empresa; fin de la talla comodín (`talla_id NULL` = "sin variante", con `talla_ref` generada para la unicidad); migraciones `..._000020` (consolidación forward-only) y `..._000021` (absorbe "Uniforme / Prenda").
- Bloque C · Etapa 2 — **activos serializados**: entidad `UnidadActivo` (serie/IMEI/etiqueta/MAC), entrada unidad por unidad, entrega/devolución por unidad, concurrencia.
