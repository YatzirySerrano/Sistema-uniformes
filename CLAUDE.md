...
- **Tipo de activo** (`tipos_activo`): catálogo por empresa con **CRUD completo** (`TipoActivoController`, pantalla `Activos/Catalogos.vue`, permiso `tipos-activo.administrar`). Tipos base sembrados: Prenda, Equipo de cómputo, Dispositivo móvil, Electrónico, Accesorio, Herramienta / Equipo, Otro. **No existe "Uniforme"** como tipo (un uniforme es un conjunto de activos). En el formulario de Activo es un **combobox con búsqueda** (`GET tipos-activo/buscar?empresa_id=&q=`, dependencia `empresa_id`) con la opción **"+ Crear nuevo tipo"** dentro del propio desplegable (sólo con permiso) → diálogo → `POST tipos-activo/rapido` (JSON). Ya **no** hay botón "Otro" permanente ni `<select>` plano.
- **Categoría de activo** (`categorias_activo`): catálogo real por empresa (`CategoriaActivoController`, permiso `categorias-activo.administrar`), sustituye al texto libre. `tipo_activo_id` nullable (relación **opcional**). `activos.categoria_id` es la fuente de verdad; `activos.categoria` (texto) se conserva como **espejo temporal** sincronizado por `ActivoController`. En el formulario de Activo es un **combobox con búsqueda** (`GET categorias-activo/buscar?empresa_id=&q=&tipo_activo_id=` — el `tipo_activo_id` prioriza y acota a ese tipo + las categorías sin tipo) con **"+ Crear nueva categoría"** dentro del desplegable → diálogo (nombre + tipo relacionado opcional) → `POST categorias-activo/rapido`.
- **Tipo y categoría son OPCIONALES e independientes**: un activo se guarda con ambos, sólo uno, o ninguno. Coherencia (validada en backend + UI): si el activo lleva tipo y categoría, y la categoría está ligada a un tipo concreto, deben coincidir. Elegir una categoría con tipo cuando el activo no tiene → autocompleta el tipo; cambiar el tipo a otro concreto → quita una categoría incompatible.
- **Unicidad normalizada** de nombres de catálogo: `tipos_activo` y `categorias_activo` tienen `nombre_normalizado` (minúsculas + espacios colapsados) con índice único `(empresa_id, nombre_normalizado)`; "Prenda" / " prenda " / "PRENDA" son el mismo nombre dentro de una empresa (empresas distintas sí pueden repetir). El modelo lo sincroniza en cada `save()` (`App\Models\Concerns\NombreNormalizado`, `App\Soporte\NormalizadorNombre`).
- **Desactivar** un tipo/categoría (diálogo de confirmación en `Activos/Catalogos.vue`) sólo lo saca de nuevas selecciones (los buscadores muestran únicamente los activos); nunca borra ni pone la FK a null: los activos existentes conservan su tipo/categoría y su pantalla de edición los muestra aunque estén inactivos (`seleccion` en el payload de `edit`).
